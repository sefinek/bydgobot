...
module.exports = async msg => {
	...
	if (msg.content.length > REQUIRED_LENGTH) {
		try {
			...
			const thread = await msg.startThread({ name: `[${msg.author.username}]: Komentarze`, reason: `Użytkownik ${msg.author.tag} (${msg.author.id}) wysłał swoje przedstawienie się.`, autoArchiveDuration: 4320 });
			thread.send({ embeds: [
				new EmbedBuilder()
			...
			dmDeliveryMsgError.aboutMe(msg, err, 15000);
		}
		return;
	}
	...
	return msg.channel.send({ embeds: [new EmbedBuilder()
		...
		.setFooter({ text: 'Jeśli chcesz skomentować jakiekolwiek przedstawienie się - zrób to w wątku.' })],
	}).then(del => {
		msg.delete();
